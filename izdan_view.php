...
function del_run(tmp,works_name)
  {
   if(confirm('Удалить пособие: \"'+works_name+'\" ?')) {window.location.href=tmp;}  }
...
var err=false;
	...
	for (i=0;i<7;i++)
	{
	 try {val_tmp_i=document.getElementById('num'+i).value;}
	 catch (e) {val_tmp_i=document.all['num'+i].value;}
	 for (j=0;j<7;j++) 	{
		if (i!=j) 	{
	...
	if (err==false) {liter_params.submit(); }
} 
</script>
...
$cnt_izdan=0;$kadri_fios='';  
  if ($izdan_id>0) {
	  ...
	  {
	  	$cnt_izdan=mysql_num_rows($res1);
		if (intval($b['kadri_id'])==intval($auth_id)) {$b['fio_short']='<b>'.$b['fio_short'].'</b>';}
		
		if (trim($authors_all)=='') {$authors_all=trim($b['fio_short']);}
		else {
...
//---------------------------------------------------------------------------------
	if ($izdan_id>0 && $_GET['action']=='delete')       {
		   ...
		   if ($kadri_id>0) 
				...
				else {echo "<div class=warning> издание у сотрудника с id=".intval($kadri_id)." не удалено</div>";$err=true;}
		   else 
		   if ($izdan_id>0)
		   {
		   ...
		   if (mysql_num_rows($res_)==0) 
		   	{
			if (mysql_query('delete from izdan where id='.$izdan_id))	//данного издания ни у кого нет
		   	{echo '<div class=success> издание удалено успешно </div>';	}
			else {echo '<div class=warning> издание не удалено</div>';$err=true;} 
			}
			else {echo '<div class=warning> издание не удалено. Удалите пособия у всех авторов/соавторов.</div>';$err=true;}	
	       }
...
if ($q!='') {$search_query=' and (izdan.name like "%'.$q.'%" or 
    	...
    	if ($kadri_id!=0) {
      	...
      	//exit;
   		...
   		if (!isset($_GET['save']) && !isset($_GET['print']))
			...
			//$res_izdan=mysql_query($query_izdan);
			//$a_izdan=mysql_fetch_array($res_izdan);
			...
			echo" <div style='text-align:right;'>
				...
				else {echo "<a class=text href='?kadri_id=".$kadri_id."' title='Публикации, ф.16'>Публикации, ф.16</a> </div> ";}
...
if ($filt_str_display!='') {echo '<div class=text><img src="images/filter.gif" alt="фильтр" border=0>включена фильтрация по: <b style="color:#FF0000;">'.$filt_str_display.'</b> &nbsp; &nbsp; <a href="?kadri_id=all"> сбросить фильтр <img src="images/del_multi_filter.gif" alt="сбросить фильтр" border=0> </a></div>';}
...
if (!isset($_GET['default'])) {//если настройки не по умолчанию
	...
	for ($j=0;$j<$num_rows;$j++) 
	{
		for ($i=0;$i<$num_rows;$i++) {
			if ($import_arr[$j]==$izdan_array[$i][0]) {$tmp_arr[$j]=$izdan_array[$i];}
									 }
	}
	$izdan_array=$tmp_arr;
...
if (isset($_POST) && count($_POST)>0) {
	for ($i=0;$i<count($_POST);$i++) 
	{
		...
		if ($value==''){$value=$izdan_array[$i][0];} else {$value=$value.','.$izdan_array[$i][0];}
	}
...
$a['authors_all']=authors_sec($a['izdan_id'],$a['authors_all'],'',0);
	...
	for ($j=count($izdan_array)-1;$j>=0;$j--) 	{
		if (trim($a[$izdan_array[$j][0]]!='')) {$str_out=$izdan_array[$j][1].trim($a[$izdan_array[$j][0]]).', '.$str_out;}
											}
	$str_out=trim($str_out);
	if (substr($str_out,-1)==',') {$str_out=substr($str_out,0,-1);}
...
<table border="1"  class=text width=99% cellspacing=0 cellpadding=0>
  <tr <?php echo $bgcolor_title;?> >
	...
	?>
    <?php if ($write_mode===true) {?><td width=31 class=notinfo><img src="images/toupdate.png" alt="изменить"></td>
	<td width=31 class=notinfo><img src="images/todelete.png" alt="изменить"></td><?php } ?>
	<td width="31">№ п\п</td> 
	...
	<td>			<?php echo print_col(8,'Копия издания');?></td></tr>
	<?php
	
        $i=0;
        while($a=mysql_fetch_array($res))  {
        $i++;
        if (($i/2)<>round($i/2)) {echo "<tr ".$bgcolor_row1." valign=top>\n";}
        else {echo "<tr  ".$bgcolor_row2." valign=top>\n";}
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////        
		if ($write_mode===true) {
		...
		$tmp_str1='';
		if (trim($a['page_range'])!='') {$tmp_str1=" (".trim($a['page_range']).")";}
		
		$tmp_str2='-';
		if (trim($a['volume'])!='') {$tmp_str2=trim($a['volume']);}
...
$a['authors_all']=authors_sec($a['izdan_id'],$a['authors_all'],'',$a['izdan_kadri_id']);
  ...
  if ($cnt_izdan>0) {
    echo "<a href='autors.php?izdan_id=".$a['izdan_id']."&kadri_id=".$kadri_id."' target='_blank' title='".$kadri_fios."'>".$cnt_izdan."</a>";
		...
		if ($server_encoding!='') $izdan_file=mb_convert_encoding($a['copy'],$server_encoding,$browser_encoding);
