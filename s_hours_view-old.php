...
if ($_SESSION['userType']=='lect' && !$lect_control) {$lect_control=false;}
...
//session_start();
//print_r($_SESSION);
if (!isset($_GET['kadri_id']) || $_GET['kadri_id']=="")
{	if ($_SESSION['userType']=='lect') 
		{header('Location:?kadri_id='.$_SESSION['kadri_id'].$kind_type_defaults);}
	else {
	  header('Location:s_hours.php');
	  echo '<hr> userType='.$_SESSION['userType'].'<hr>';};
 //header('location:s_hours.php');
 
 echo $_GET['kadri_id']."Не найден преподаватель. <a href='s_hours.php'>Вернуться к выбору преподавателя...</a>";exit;}

//exit();
//echo '1111111111';
//print_r($_SESSION);
if ($_SESSION['userType']=='lect' && (!isset($_GET['kadri_id'])|| $_GET['kadri_id']!=$_SESSION['kadri_id'])) 
	{header('Location:?kadri_id='.$_SESSION['kadri_id'].'&year=2'.$kind_type_defaults);}
//else { if (!isset($_GET['kadri_id'])) {header('Location:s_hours.php'); }	 }
//if ($_SESSION['userType']=='lect' && $_SESSION['role']!='izdan' && (trim($_GET['id_kadri'])!=trim($_SESSION['kadri_id']) 
...
header('Location:?kadri_id='.$_GET['kadri_id'].'&year='.$_GET['year'].$kind_type_defaults);
...
<html>
<head>
<title>Часовая нагрузка сотрудников</title>
...
echo '<form name="h_copy_'.$part_num.'" method="POST" action="?kadri_id='.$_GET['kadri_id'].'&year='.$_GET['year'].'&export'.$kind_type_defaults.'"><table name=tab1 border=1 cellpadding="0" cellspacing="0" width="">
	...
	if ($_SESSION['userType']!='admin') {$cols_disable=2;}
...
//	echo '<h3>$total='.$total.'</h3>';
	...
	while ($tmpval=mysql_fetch_array($res_all))	//вывод показателей
	{
		...
		echo '<td align=right>'.$tmpval['stud_cnt'].'&nbsp</td>';//число студентов
		...
		for ($i=0;$i<sizeof($hour_kind_code);$i++) 
			{
			if ($tmpval[$hour_kind_code[$i]]==0) {$tmpval[$hour_kind_code[$i]]='';}
		...
		if (isset($_GET['filial_flag']) && $_GET['filial_flag']=='on')	//вывод с учетом выезда
			...
			{echo '<td align="center">'.($sum*0.5).'&nbsp</td>';$sumColFilial+=$sum*0.5;}
...
//-------------------------	
		if ($sumCol[$i]!=$sum_l_h) {echo '<td  align="center">'.$sumCol[$i].'<span class=warning>'.$sum_l_h.'</span></td>';}
			else {echo '<td  align="center">'.$sumCol[$i].'</td>';}
		}
		else {echo '<td  align="center">'.$sumCol[$i].'</td>';}		
...
<select id="teach_name" name="teach_name" style="width:200;"><?php 
		if ($_SESSION['userType']=='admin' || $_SESSION['role']=='izdan') {
		 $listQuery='select k.id,k.fio from kadri k left join person_types pt on pt.id=k.person_type 
			...
			order by fio ';}
		 else {$listQuery='select k.id,k.fio from kadri k left join person_types pt on pt.id=k.person_type 
...
while (list($key, $value) = each ($_POST)) {
 	if 	  (strstr($key,"checkbox_h_copy_")) {
	  ...
	  //echo ' strpos='.strpos($key,'checkbox_h_copy_').'<br>';
	...
	$query='SELECT `subject_id` , `spec_id` , `level_id` , `group_id` , `hours_kind_type` , `groups_cnt` , 
		`stud_cnt` , `lects` , `practs` , `labor` , `rgr` , `recenz` , `kurs_proj` , `consult` , `test` , `exams`, 
		`study_pract` , `work_pract` , `consult_dipl` , `gek` , `aspirants` , `aspir_manage` , `duty`  
	FROM `hours_kind` WHERE `id` ='.$mail_id.' limit 0,1';	/*, `kadri_id` , `year_id` , `part_id`, `comment`,`id` */
	...
	while (list($key_, $value_) = each ($a)) {$str_fields=$str_fields.', `'.$key_.'`';$str_values=$str_values.', "'.$value_.'"';}
	//print_r($a);
...
$fio_res=mysql_query("select kadri.fio,kadri.fio_short,dolgnost.name as dolgnost_name from kadri left join dolgnost on kadri.dolgnost=dolgnost.id where kadri.id=".$_GET['kadri_id']." limit 0,1");
...
<table width="" border="0">
  ...
  <tr valign="top" class="text">
	...
	<div align=right><sup>должность преподавателя</sup></div>
	 <div align=right>';
	 	if (!isset($_GET['save']) && !isset($_GET['print']) && $_SESSION['userType']!='lect') {
		
	 	//$query_string=reset_param_name($query_string,'kadri_id');
	 	
		 ?><select id="kadri_id" name="kadri_id" 
		 onChange="javascript:window.location.href='?kadri_id='+this.options[this.selectedIndex].value+'&<?php echo reset_param_name($query_string,'kadri_id');?>';"><?php 
		$listQuery="select k.id,k.fio_short as name 
		 	from kadri k left join person_types pt on pt.id=k.person_type where pt.name_short like '%ППС%' 
...
if (!isset($_GET['save']) && !isset($_GET['print'])) {
		...
		$res=mysql_query($query);
		echo '<form name=kind_type_form action="" method="get">';
		while ($a=mysql_fetch_array($res)) 
			{$select_val='';
			 if (isset($_GET['kind_type'.$a['id']]) && $_GET['kind_type'.$a['id']]=='on') {$select_val=' checked'; } 
			 echo '<input type=checkbox name="kind_type'.$a['id'].'"'.$select_val.'>'.$a['name'].' &nbsp;';			
			}
		?>
		...
		<?php
		echo '	
		<input type=hidden name=kadri_id value="'.$_GET['kadri_id'].'">
...
//-------------------------------------------------------------------------------------------------------------
	...
	echo '<b>Всего за год: <u>'.$total_year;
		//с учетом выезда
		if (isset($_GET['filial_flag']) && $_GET['filial_flag']=='on') {echo '<small> ( с учетом филиалов: '.$total_year_fil.')</small> ';}	
	echo '</u></b>';
	}
else {
	//осенний семестр 2006-2007 года
	echo '<span align="center">  <u>год: '.'2006-2007'.'</u></span>';
	print_hours('2006-2007','осенний');
	echo '<br>';
	//echo '<p>&nbsp;</p>';
	print_hours('2006-2007','весенний');}
?>
<br><br>
<table cellspacing="0" cellpadding="0" border="0" class=text><tr>
  	  <td width="30"></td>
      ...
      <td width="30"></td>
      <td>	Преподаватель____________<?php echo $fio_short;?> </td>
</tr></table>
...
<?php
echo '<p><a href="s_hours.php?tab=2&kadri_id='.$_GET['kadri_id'].'">Вернуться...</a><p>';} ?>
