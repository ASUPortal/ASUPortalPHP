...
if (isset($_GET['getsubj']) && intval($_GET['getsubj'])>0) {
    $getsubj=intval($_GET['getsubj']);
}

$getdir=0;	// папка с учебными материалами выбранного преподавателя
if (isset($_GET['getdir']) && intval($_GET['getdir'])>0) {
    $getdir=intval($_GET['getdir']);
}
...
if ($getdir>0) {
    ...
    FROM
        (documents documents
            INNER JOIN
                subjects subjects ON
                    (documents.subj_id = subjects.id)
        )
    INNER JOIN
        users users ON
    ...
    if (!$hide_person_data_rule && $vasubj[0]['user_name']!='') {
        $head_title=$vasubj[0]['user_name'].'. '.$head_title;
    }
    if ($vasubj[0]['subj_name']!='') {
        $head_title=$vasubj[0]['subj_name'].'. '.$head_title;
    }
} else if ($getsubj>0) {
        $fiosubj=getScalarVal('
            select
                name
            from
                subjects
...
function letters() {
    ...
    if (isset($_GET['getsub']) && intval($_GET['getsub'])>=0) {
        $letterId=intval($_GET['getsub']);
    }

    echo '
        <p class=middle_library style="font-size:18pt;">
            <span class=text>предметы с русскими начальными буквами: </span>
        </p>';
    ...
    if (mysql_num_rows($res_rus)>0) {
        ...
        echo '<div class=text> <b>файлов не найдено.</b></p>';
    }
    echo '<p><span class=text>предметы с англоязычными начальными буквами: </span> ';
    $query_letter_rus='
    SELECT
        UPPER(left(s.name,1)) as name,
        COUNT(f.id_file) AS file_cnt
    FROM
        (documents d
            INNER JOIN
	              files f ON
	                  (d.nameFolder = f.nameFolder)
        )
    INNER JOIN
        subjects s ON
            (s.id = d.subj_id)
    ...
    if (mysql_num_rows($res_rus)>0) {
        ...
        echo '<span class=text> <b>файлов не найдено.</b></span></p>';
    }
    echo '<a class=main href="?onget=1&getallsub=1">все</a>';

    ?>
    <div class=text>
        <a href="#show" onclick="hide_show('nFiles');">последние файлы в разделе "Учеба" (за 7 дней)  показать/скрыть</a>
    </div>
    <?php
    ...
    if (mysql_num_rows($res)>0) { ?>
        ...
        while ($a=mysql_fetch_array($res)) {
            ...
            <tr align=left>
	            ...
	            <td>
	                <a href="?onget=1&getdirect='.$a['nameFolder'].'&getfile='.$a['nameFile'].'">';
                    echo file_type_img('library/'.$a['nameFolder'].'/'.$a['nameFile']);
                    echo '<b>'.$a['browserFile'].'</b> ('.$a['entry'].') </a>
                </td>
	            <td>
        ...
        echo '<div class=warning>новых файлов не обнаружено</div>';
    }
}
...
if (isset($_GET['getdirect']) ) {
    ...
    set
        entry=entry+1
    where
        nameFolder="'.f_ri($_GET['getdirect']).'"
...
if (isset($_SESSION['auth']) && $_SESSION['userType']=='преподаватель') {
    echo '<a href="lect_library.php" class=text title="'.$_SESSION['FIO'].'"> добавить  свой предмет\файл</a>';
}
echo '</div>
	<div class=text style="text-align:center;"> часть файлов сжаты архиватором WinRar, загрузить архиватор Вы можете <a href="apps/wrar34b5ru.exe"> <u>здесь</u> </a>';
letters();
echo '</div>';
if ($getdir>0) {
    //выбор пособий преподавателя по предмету
