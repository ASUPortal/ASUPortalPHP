...
ALTER TABLE `study_marks` ENGINE = InnoDB;

CREATE TABLE `pl_corriculum_workplan_brs` (
`id` int(11) NOT NULL,
  `plan_id` int(11) NOT NULL,
  `mark_id` int(11) NOT NULL,
...
ALTER TABLE `pl_corriculum_workplan_brs`
 ADD PRIMARY KEY (`id`), ADD KEY `plan_id` (`plan_id`), ADD KEY `mark_id` (`mark_id`);
...
ALTER TABLE `pl_corriculum_workplan_brs`
ADD CONSTRAINT `pl_corriculum_workplan_brs_ibfk_2` FOREIGN KEY (`mark_id`) REFERENCES `study_marks` (`id`) ON UPDATE CASCADE,
ADD CONSTRAINT `pl_corriculum_workplan_brs_ibfk_1` FOREIGN KEY (`plan_id`) REFERENCES `pl_corriculum_workplans` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
