...
class CWorkPlanTermSectionsSecondFinalControl extends CAbstractPrintClassField {
    public function getFieldName()
    {
        return "Вид итогового контроля для второго семестра в списке";
    }
    ...
    public function execute($contextObject)
    {
        ...
        foreach ($finalControls->getItems() as $control) {
        	if (CBaseManager::getWorkPlanTerm($control["termId"]) == $terms[1]) {
        		$result = $control["name"];
        	}
        }
        return $result;
