...
class CPractPlacesController extends CBaseController{
    ...
    public function actionIndex() {
        ...
        if (!is_null(CRequest::getFilter("name"))) {
        	$query->condition("practic.id = ".CRequest::getFilter("name"));
        }
        // фильтр по комментарию
        if (!is_null(CRequest::getFilter("comment"))) {
        	$query->condition("practic.id = ".CRequest::getFilter("comment"));
        }
        $towns = array();
    ...
    public function actionSearch() {
    	...
    	*/
    	$query = new CQuery();
    	$query->select("distinct(practic.id) as id, practic.name as name")
    	->from(TABLE_PRACTICE_PLACES." as practic")
    	->condition("practic.name like '%".$term."%'")
    	->limit(0, 5);
    	 ...
    	 */
    	$query = new CQuery();
    	$query->select("distinct(practic.id) as id, practic.comment as comment")
    	->from(TABLE_PRACTICE_PLACES." as practic")
    	->condition("practic.comment like '%".$term."%'")
    	->limit(0, 5);
    	foreach ($query->execute()->getItems() as $item) {
    		$res[] = array(
    				"label" => $item["comment"],
