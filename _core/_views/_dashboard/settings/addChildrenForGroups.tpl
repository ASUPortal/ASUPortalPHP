...
{block name="asu_center"}
    <h2>Добавление дочернего элемента для группы: {CStaffManager::getUserGroup(CRequest::getInt("id"))->comment}</h2>
...
<form action="settings.php" method="post" class="form-horizontal">
        {CHtml::hiddenField("action", "addChildrenForGroupsProcess")}
	...
	<div class="control-group">
        {CHtml::activeLabel("parent_id", $form)}
        <div class="controls">
            {CHtml::activeDropDownList("parent_id", $form, $parents)}
            {CHtml::error("parent_id", $form)}
        </div>
    </div>
