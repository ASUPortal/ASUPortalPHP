    ...
    {else}
        <table class="table table-striped table-bordered table-hover table-condensed">
            ...
            {foreach $objects->getItems() as $object}
                <tr>
                    <td><a href="#" class="icon-trash" onclick="if (confirm('Действительно удалить вид контроля')) { location.href='workplanintermediatecontrol.php?action=delete&id={$object->getId()}'; }; return false;"></a></td>
                    <td>{$object->ordering}</td>
                    <td><a href="workplanintermediatecontrol.php?action=edit&id={$object->getId()}" class="icon-pencil"></a></td>
                    ...
                    {/if}
                    <td>{$object->controlType->getValue()}</td>
                </tr>
            {/foreach}
            </tbody>
        </table>
