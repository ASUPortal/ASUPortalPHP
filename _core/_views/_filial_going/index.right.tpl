...
<div id="printDialog" class="modal hide fade">
    ...
    <div class="modal-body">
        <b>Печать для всех командировок</b>

        {CHtml::printGroupOnTemplate("formset_filial_goings")}
    </div>
</div>
...
<script>
    function printWithTemplate(manager, method, template_id) {
         ...
         */
        var filial_goings = new Array();
        jQuery.each(jQuery("input[name='selectedDoc[]']:checked"), function(key, value){
        	filial_goings.push(jQuery(value).val());
        });
        /**
         * Адаптируем прогресс-бар
         */
        jQuery("#progressbar").attr("items", (filial_goings.length - 1));
        jQuery("#progressbar").css("width", "0%");
        /**
         * Для каждой командировки генерим шаблон
         */
        var attachments = new Array();
        jQuery.each(filial_goings, function(key, value) {
            jQuery.ajax({
                dataType: "json",
                 ...
                 */
                if (attachments.length == filial_goings.length) {
