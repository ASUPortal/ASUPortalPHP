...
<form action="index.php" method="post" id="loadsFall">
    ...
    <table rel="stripe" class="table table-striped table-bordered table-hover table-condensed table-load" border="1">
        <tr>
            ...
            <th style="vertical-align:middle; text-align:center;">{CHtml::activeViewGroupSelect("id", $loadsFall->getFirstItem(), true)}</th>
            <th style="vertical-align:middle; text-align:center;">{CHtml::tableOrder("discipline_id", $loadsFall->getFirstItem(), false, false)}</th>
            <th style="vertical-align:bottom;"><div class="vert-text">Факультет</div></th>
            <th style="vertical-align:bottom;"><div class="vert-text">{CHtml::tableOrder("speciality_id", $loadsFall->getFirstItem(), false, false)}</div></th>
            <th style="vertical-align:bottom;"><div class="vert-text">{CHtml::tableOrder("level_id", $loadsFall->getFirstItem(), false, false)}</div></th>
            <th style="vertical-align:bottom;"><div class="vert-text">{CHtml::tableOrder("groups_count", $loadsFall->getFirstItem(), false, false)}</div></th>
            <th style="vertical-align:bottom;"><div class="vert-text">{CHtml::tableOrder("students_count", $loadsFall->getFirstItem(), false, false)}</div></th>
            <th style="vertical-align:middle; text-align:center;">Учебные группы</th>
            <th style="vertical-align:middle; text-align:center;">{CHtml::tableOrder("load_type_id", $loadsFall->getFirstItem(), false, false)}</th>
            <th style="vertical-align:middle; text-align:center;">{CHtml::tableOrder("comment", $loadsFall->getFirstItem(), false, false)}</th>
            {foreach $loadsFall->getFirstItem()->getStudyLoadTable()->getTableTotal() as $typeId=>$rows}
        ...
        <tr>
	        ...
	        {else}
	            {$ths = 11}
	        {/if}
	        {for $i=1 to $ths + count($loadsFall->getFirstItem()->getStudyLoadTable()->getTableTotal())}
