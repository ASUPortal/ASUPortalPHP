...
{block name="asu_center"}
    <h2 style="text-align:center">Расписание по преподавателям</h2>
	...
	<div class=text style="text-align:center">
		<div style="font-size:18pt;">
			{if ($resRusLetters)}
	        	{foreach $resRusLetters as $name=>$count}
	        		{if (array_key_exists($letterId, $firstLet))}
	        			...
	        			{else}
	        				<a href="?action=lecturers&getsub={array_search({$name},$firstLet)}" title="в категории записей: {$count}">{$name}<font color=#e70013><sub>{$count}</sub></font></a>
	        			{/if}
	        		{else} 
	        			<a href="?action=lecturers&getsub={array_search({$name},$firstLet)}" title="в категории записей: {$count}">{$name}<font color=#e70013><sub>{$count}</sub></font></a>
	        		{/if}
	        ...
	        {/if}
	        <br><br><a href="?action=lecturers&getallsub=1">все</a><br><BR>
		</div>
	</div>
    ...
    {else}
        ...
        <table class="table table-striped table-bordered table-hover table-condensed">
		    ...
		    <tr>
		    	<td>{counter}</td>
		        <td><a href="{$web_root}_modules/_lecturers/index.php?action=view&id={$lect->getUser()->id}" title="О преподавателе">
			        {if (CSettingsManager::getSettingValue("hide_personal_data"))}
			        	{$lect->FIO}</a>
			        {else}
			        	{$lect->fio}</a>
