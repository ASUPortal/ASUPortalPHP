        ...
        {foreach $items->getItems() as $key=>$value}
            {if $multiple}
                <label class="checkbox">
                    <input type="checkbox" value="{urlencode($value)}" name="selected[{$key}]"/>
                    {$value}
                </label>
            {else}
                <label class="radio">
                    <input type="radio" value="{urlencode($value)}" name="selected[]" {if !$selectedFirst}checked{$selectedFirst = true}{/if}/>
                    {$value}
                </label>
            {/if}
        {/foreach}
