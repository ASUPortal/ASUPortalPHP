...
//---------------------------------------------Раздел 1--- перенесен в центр.задачу "Справочники" портала--

/*echo '<a class="notinfo" href="?id_razdel1">1. Справочник учебная работа</a><br>';
if (isset($_GET['id_razdel1']))
 ...
 if ($_POST['work']=='')
    {
 	...
 	{
 	$sql="INSERT INTO spravochnik_uch_rab (id,name) VALUES ('".$_POST['max']."','".$_POST['work']."')";
 	$add1="ALTER TABLE `plan` ADD `rab_".$_POST['max']."` INT(3) NOT NULL";
    $add2="ALTER TABLE `fact` ADD `rab_".$_POST['max']."` INT(3) NOT NULL";
 	mysql_query($sql) or die("Возникла ошибка добавления : ".mysql_error());
 	mysql_query($add1) or die("Возникла ошибка добавления : ".mysql_error());
 	mysql_query($add2) or die("Возникла ошибка добавления : ".mysql_error());
 ...
 foreach ($_POST['delete_rab'] as $i)
  {
  $sql="DELETE FROM spravochnik_uch_rab WHERE id='$i'";
  $del1="ALTER TABLE `fact` DROP `rab_".$i."`";
  $del2="ALTER TABLE `plan` DROP `rab_".$i."`";
  mysql_query($del1) or die("Возникла ошибка удаления : ".mysql_error());
  mysql_query($del2) or die("Возникла ошибка удаления : ".mysql_error());
  mysql_query($sql) or die("Возникла ошибка удаления : ".mysql_error());
  echo'<h3>Данные успешно удалены</h3>';
  }
 }
 ...
 if((isset($_POST['edit']))and (isset($_POST['delete_rab'])))
			{
					...
					echo "<table class=indplan border=1><tr class=indplan><th>Список учебных работ</td></tr><br>";
					echo "<FORM method=POST>";
			foreach ($_POST['delete_rab'] as $i)
				{
 					echo "<input name=delete_rab[".$i."] type='hidden' value=".$i.">";

 					$sql="SELECT * FROM spravochnik_uch_rab WHERE id=".$i."";
 					$mysql=mysql_query($sql);
 					$name=mysql_fetch_row($mysql);
 					echo "<tr>
					<td><textarea name='name".$i."' rows=4 cols=38 maxlength='200'>$name[1]</textarea></td></tr>";
				}
 				...
 				foreach ($_POST['delete_rab'] as $i)
					{
	 					...
	 					if ($name!='') //проверка на пустые значения
                               {
							$sql="UPDATE spravochnik_uch_rab SET name='$name' WHERE id=$i";
				...
				<th>Список учебных работ</td>
			       ...
			       <FORM method=POST>";
				    ...
				    echo "<input name=delete_rab[".$i."] type='hidden' value=".$i.">";
 					...
 					$name=mysql_fetch_row($mysql);
 					echo "<tr>
 					<td><textarea name='name".$i."' rows=4 cols=38 maxlength='200'>$name[2]</textarea></td></tr>";
				    		}
				    		echo "</table><input type='submit' name='izmen' value='Изменить'></FORM>";
           				   }
                }
 ...
 {
 $max=0;
 $sql="SELECT MAX(id) FROM spravochnik_uch_rab";
 $mysql=mysql_query($sql);
 ...
 <textarea name="work" rows=4 cols=30 maxlength="200">Название работы</textarea>
 <br><input type="submit" name="save" value="Добавить">
 </form>';

 }

//---------------------------------------------Вывод-----------------------------------------
 $sql="SELECT id,name FROM spravochnik_uch_rab ORDER BY id ";
 $mysql=mysql_query($sql);
 $check=mysql_num_rows($mysql);
 if (!$check)
 echo'<h3>Справочник учебных работ пуст</h3>
 ...
 <table border=1 class=indplan><tr class=indplan><th>Список учебных работ</td><th></td></tr>';
 while($name=mysql_fetch_array($mysql))
 {
 echo '<tr onmouseover=this.style.background="#fffafa" onmouseout=this.style.background="#DFEFFF"><td>'.$name['name'].'</td><td><input type="checkbox" name="delete_rab[]" value="'.$name['id'].'"></td></tr>';
 }
 echo '</table>';
 echo '</form>';
