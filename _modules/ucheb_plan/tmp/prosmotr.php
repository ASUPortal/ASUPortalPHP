...
//---------------------------------------------Раздел 1--------------------------------------
if (!isset($_GET['save']) && !isset($_GET['print']))
{echo '<a class="notinfo" href="?id_kadri='.$id_kadri.'&id_year='.$id_year.'&id_razdel1">1. Учебная работа</a>';}
...
$i=1;
While ($name=mysql_fetch_array($uch_rab))
 	{
 $n=$name[0];
    //--------------------------------------План на семестр
 	$q_val='select sum('.$name['name_hours_kind'].')+sum('.$name['name_hours_kind'].'_add)  
        from hours_kind 
        where kadri_id="'.$id_kadri.'"
            ...
            //echo $q_val;
        //print_r($name);
        $resp1['rab_'.$n.'']=getScalarVal($q_val);  
	...
	$q_val='select sum('.$name['name_hours_kind'].')+sum('.$name['name_hours_kind'].'_add)  
        ...
        where kadri_id="'.$id_kadri.'" 
            ...
            //echo $q_val;
        //print_r($name);
        $resp2['rab_'.$n.'']=getScalarVal($q_val);  
    //--------------------------------------
 	echo'<tr onmouseover=this.style.background="#fffafa" onmouseout=this.style.background="#DFEFFF"><td>'.$name[1].'</td>
 	<td align=right>'.numLocal(number_format($resp1['rab_'.$n.''],$comma_cnt)).'</td>';
 $summa=0;$i=1;
while (($i)!=6)
		{
$sqlm="SELECT * FROM fact WHERE id_month=".$i." and id_kadri=".$id_kadri." and id_year=".$id_year."";
  $mysqlm=mysql_query($sqlm);
  $res=mysql_fetch_array($mysqlm);
echo '<td align=right>'.numLocal(number_format($res['rab_'.$n.''],$comma_cnt)).'</td>';
 $i++;
 $summa=$summa+$res['rab_'.$n.''];
		}

echo '<td align=right>'.numLocal(number_format($summa,$comma_cnt)).'</td>';
echo'<td align=right>'.numLocal(number_format($resp2['rab_'.$n.''],$comma_cnt)).'</td>';
...
echo '
<td align=right>'.numLocal(number_format($summa,$comma_cnt)).'</td></tr>';
...
//------------------Редактирование "1. Учебная работа"-------------------------
...
echo'<form method="POST" action="?'.reset_param_name($_SERVER['QUERY_STRING'],'load1').'">
 <table border=1 class=indplan cellpadding=2 cellspacing=0>
 <tr class=indplan>
  <th rowspan="3">Название работ</td>
  <th rowspan="3"><font size=-1 TITLE="значения заносятся из нагрузки автоматически" style=cursor:help>План
...
while (($i)!==6)    // обход месяцы 1-6 (сент-янв) - столбцы
{
    if (isset($_GET['load1']) && $_GET['load1']==1) // загрузка усредненных данных по плану
    {
        ...
        $sqlm="SELECT `rab_{$n}` FROM fact
            WHERE id_month=".$i." and id_kadri=".$id_kadri." and id_year=".$id_year."";
        $mysqlm=mysql_query($sqlm);
        $row=mysql_fetch_assoc($mysqlm);
        $res["rab_{$n}"]=$row["rab_{$n}"];
    }
    echo '<td>
    <input style="text-align:right;" type="text" name="id'.$name[0].'m'.$i.'" id="id'.$name[0].'m'.$i.'" 
...
echo '
<td style="text-align:right;" class="'.($summa!=$resp1['rab_'.$n.'']?'warning':'success').'"><span name="id'.$name[0].'_sum1">'.numLocal(number_format($summa,$comma_cnt)).'</span></td>';
...
echo '<td><input type="text" style="text-align:right;" name="id'.$name[0].'m'.$i.'" id="id'.$name[0].'m'.$i.'" 
value="'.numLocal(number_format($res['rab_'.$n.''],$comma_cnt)).'" size="4"
onChange="javascript:calc_sum(\'id'.$name[0].'m'.$i.'\');" ></td>';
 $i++;
 $summa=$summa+$res['rab_'.$n.''];
}
echo '
<td style="text-align:right;" class="'.($summa!=$resp2['rab_'.$n.'']?'warning':'success').'"><span name="id'.$name[0].'_sum2">'.numLocal(number_format($summa,$comma_cnt)).'</span></td></tr>';
...
echo'</table><br>
    ...
    <ul><li><span class=warning>Красным</span> цветом в столбце "итого" отмечены несовпадающие с планом фактические значения, <span class=success>зеленым</span> - совпадающие.</li></ul>
    
    </div>
    <input type="submit" name="main" value="Сохранить"> &nbsp; 
    <input type="button" name="main" value="Загрузить" title="загрузка из нагрузки за выбранный период"
        onclick="confirm_url(document.location.href=\'?'.reset_param_name($_SERVER['QUERY_STRING'],'load1').'&load1=1\');"> &nbsp;
    <input type="button" name="main" value="Вернуть" title="вернуть данные до сохранения"  
